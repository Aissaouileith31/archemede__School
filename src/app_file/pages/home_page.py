...
# Change these lines in your code:
    ...
    async def request_notification_permission():
        # This opens the Android System Dialog
        permission_status = await ph.request_permission(fph.PermissionType.NOTIFICATION)
    ...
    more_tab = ft.Container(
        ...
        content=ft.Column([
            ...
            ft.ListTile(
            leading=ft.Icon(ft.Icons.NOTIFICATIONS_ACTIVE, color=PRIMARY),
            title=ft.Text("Activer les Notifications"),
            subtitle=ft.Text("Nécessaire pour recevoir les alertes de l'école"),
            on_click=lambda _: page.run_task(request_notification_permission)
            ),
